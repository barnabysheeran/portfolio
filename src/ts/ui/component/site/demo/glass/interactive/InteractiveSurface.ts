...
export default class InteractiveSurface {
  ...
  static removeBlock(uuid: string): void {
    // Added return type
  ...
  static #onClick(event: MouseEvent, callback: ClickCallback): void {
    // Added types
    const clickDataString = (event.currentTarget as HTMLElement).dataset
      .clickData;
    const clickData: IdData = clickDataString
      ? JSON.parse(clickDataString)
      : { id: '' };
    callback(clickData);
  }

  static #onRollOver(event: MouseEvent, callback: ClickCallback): void {
    // Added types
    const clickDataString = (event.currentTarget as HTMLElement).dataset
      .clickData;
    const clickData: IdData = clickDataString
  ...
  static #onRollOut(event: MouseEvent, callback: ClickCallback): void {
    // Added types
    const clickDataString = (event.currentTarget as HTMLElement).dataset
      .clickData;
    const clickData: IdData = clickDataString
